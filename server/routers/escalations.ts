...
import { createBreakdownNote, createContentSkootSuggestions, createSmartEscalation, getCreatorEscalations, getEscalationBrief, getStudentEscalations, getSupportProfiles, getStudentEscalationRoute, saveSupportProfile, updateEscalationStatus } from "../db";

const levelSchema = z.enum(["csm", "coach"]);
export const escalationsRouter = router({
  supportProfiles: protectedProcedure.query(({ ctx }) => getSupportProfiles(ctx.user.id)),
  saveSupportProfile: protectedProcedure.input(z.object({ id: z.number().int().positive().optional(), routingLevel: levelSchema, displayName: z.string().trim().min(2).max(300), bookingUrl: z.string().url().max(2048).optional(), active: z.boolean().optional() })).mutation(({ ctx, input }) => saveSupportProfile(ctx.user.id, { ...input, bookingUrl: validateBookingUrl(input.bookingUrl) })),
