...
export default function CoachOnboarding() {
  ...
  const busy = selectRole.isPending || saveProfile.isPending || saveMethod.isPending || shapeDraft.isPending || complete.isPending;
  ...
  if (stage === "launch" || stage === "complete") return <main className="auth-page"><MemphisShapes quiet /><section className="auth-shell auth-shell--signed-in"><BrandMark /><span className="auth-kicker">YOUR FIRST PACK IS READY</span><h1>Now invite the first student.</h1><p>Your Pack is versioned, private by default, and ready for the existing secure enrollment-link flow.</p><Button size="lg" onClick={() => setLocation("/creator")}><UsersRound className="size-4" /> Open my Creator launch</Button></section></main>;
