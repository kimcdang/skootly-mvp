...
export function CreatorSupportPanel() {
  ...
  const [profile, setProfile] = useState({ displayName: "", bookingUrl: "", routingLevel: "coach" as "coach" | "csm" });
  ...
  const saveProfile = trpc.escalations.saveSupportProfile.useMutation({
    onSuccess: () => { utils.escalations.supportProfiles.invalidate(); setProfile({ displayName: "", bookingUrl: "", routingLevel: "coach" }); toast.success("Support route saved."); },
  ...
  return (
    <section className="creator-support">
      ...
      <div className="creator-support__profiles">
        <div><h3>Support routes</h3><p>Add a CSM or your own booking link. Skootly never creates calendar events.</p>{profiles.data?.map(item => <p className="support-profile" key={item.id}><b>{item.displayName}</b> · {item.routingLevel}{item.bookingUrl ? " · booking link ready" : " · request-only"}</p>)}</div>
        <div className="support-profile-form"><Label>Name<Input value={profile.displayName} onChange={event => setProfile({ ...profile, displayName: event.target.value })} placeholder="Kim or Client Success" /></Label><Label>Private booking URL (optional)<Input value={profile.bookingUrl} onChange={event => setProfile({ ...profile, bookingUrl: event.target.value })} placeholder="https://…" /></Label><div><Button type="button" variant={profile.routingLevel === "csm" ? "default" : "outline"} onClick={() => setProfile({ ...profile, routingLevel: "csm" })}>CSM</Button><Button type="button" className="ml-2" variant={profile.routingLevel === "coach" ? "default" : "outline"} onClick={() => setProfile({ ...profile, routingLevel: "coach" })}>Coach</Button></div><Button disabled={!profile.displayName || saveProfile.isPending} onClick={() => saveProfile.mutate({ ...profile, bookingUrl: profile.bookingUrl || undefined })}><Plus className="size-4" /> Add route</Button></div>
      </div>
